...
import { useGetWarehousesQuery, useEditWarehouseMutation } from "../features/api/warehousesApi";
...
export const WarehouseEditpage = () => {
  ...
  // получаем все склады
  const { data: warehouses, isSuccess: isSuccessWarehouses, error, refetch } = useGetWarehousesQuery();
  ...
  // значения инпутов
  const [name, setName] = useState<string>(getWarehouseById(warehouseIdNumber)?.name || "");
  const [location, setLocation] = useState<string>(getWarehouseById(warehouseIdNumber)?.location || "");
  ...
  const inputNameHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (warehouseExists) {
      setNameMessage("Такой склад уже существует");
    } else {
      setNameMessage("");
    }
  };
  // обработчик инпута адреса
  ...
  useEffect(() => {
    ...
    setName(getWarehouseById(warehouseIdNumber)?.name || "");
    setLocation(getWarehouseById(warehouseIdNumber)?.location || "");
  }, [isSuccessWarehouses]);

  // запрос на редактирование склада
  ...
  const editWarehouseHandler = async () => {
    ...
    try {
      ...
      const response = await editWarehouse({ id: warehouseIdNumber, name: name, location: location }).unwrap();
      console.log(`Склад с именем ${name} успешно отредактирована:`, response);
      navigate("/warehouses");
    } catch (error) {
      console.error("Склад не получилось отредактировать:", error);
      setIsShowError(true);
    }
  };
      ...
      <main className="bg-starkit-magnolia">
        ...
        <div className="mt-[-35px] flex justify-center items-center">
          <div className="w-[500px] px-6 py-7 pb-7 bg-white rounded-[40px]">
            ...
            <Button onClick={editWarehouseHandler} text="Готово" />
          </div>
        </div>
        {isShowError && (
          <div className="h-dvh fixed inset-0 bg-black/50 bg-none flex justify-center items-center z-[8]">
            <div className="w-[425px] px-6 pb-6 flex flex-col justify-center items-center bg-white rounded-[40px]">
              ...
              <span className="mb-9 text-center text-black text-base font-medium">
                Склад не получилось отредактировать.<span className="text-starkit-electric"> Уже начали исправлять данную проблему.</span>
              </span>
